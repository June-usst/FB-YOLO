...
def bbox_iou(box1, box2, xywh=True, GIoU=False, DIoU=False, CIoU=False,EnEIoU=False, eps=1e-7,CombinedCIoU=False):
    """
    ...
    # IoU
    iou = inter / union
    if CIoU or DIoU or GIoU or EnEIoU or CombinedCIoU:
        cw = b1_x2.maximum(b2_x2) - b1_x1.minimum(b2_x1)  # convex (smallest enclosing box) width
        ch = b1_y2.maximum(b2_y2) - b1_y1.minimum(b2_y1)  # convex height
        if CIoU or DIoU or EnEIoU or CombinedCIoU:  # Distance or Complete IoU https://arxiv.org/abs/1911.08287v1
            ...
            if CIoU:  # https://github.com/Zzh-tju/DIoU-SSD-pytorch/blob/master/utils/box/box_utils.py#L47
                ...
                # 计算两个框的总面积
                total_area = area_b1 + area_b2 + eps  # 防止除零

                # 计算尺度不变性因子：通过每个框的面积对损失进行加权
                scale_factor_b1 = area_b1 / total_area
                scale_factor_b2 = area_b2 / total_area

                # 根据目标的面积进行加权，使得小目标对损失的贡献更大
                return iou - (scale_factor_b1 * (rho2 / c2) + scale_factor_b2 * (rho_w2 / cw2 + rho_h2 / ch2))
